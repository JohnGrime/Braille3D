...
#
# The FSM processes a list of input tokens, building and adding tokens to the
# output list (and potentially modifying the input list) and moving into a
# specified "new" state (which might simply be the previous state).
#
# A state transition is composed of:
#
# 1. An input for a given state
# 2. A tuple of (input additions, output additions, new state)
#
...
class FSM:
	...
	def add_state(self, name, transition_map_list):
		"""
	...
	def process(self, in_toks, debug=False):
		"""
	...
	def transform(self, txt, tag, state):
		"""
	...
	def postprocess(self, txt, tag, state):
		"""
